...
function parseOptions(options) {
  ...
  if (!options.headers || !(options.headers instanceof Object)) options.headers = {};
  if (typeof options.query == 'string') options.query = QS.parse(options.query);
  if (!options.query || !(options.query instanceof Object)) options.query = {};
...
function mergeOptions(target, defaults) {
  ...
  Object.keys(defaults.query).forEach(function(k) {
    if (target.query.hasOwnProperty(k)) return;
    target.query[k] = defaults.query[k];
  });
