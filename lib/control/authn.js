...
function enforce(req, res, next) {
  if (req.isAuthenticated()) {
    ...
    // Pass the user-id to the next service
    // req.headers[req.app.get('idHeader')] = req.user.id;
...
exports.enforce = function(app, options) {
  options = options || {};
  app.set('userHeader', options.userHeader || 'x-request-user');
